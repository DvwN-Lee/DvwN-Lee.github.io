...
export const fAInancialAgentContent = {
    ...
    actions: [
        '<strong>LangGraph StateGraph 기반 Agent 재설계</strong>: while loop → LangGraph StateGraph 진화 과정을 코드로 보존하며, 상태 전이와 도구 호출을 선언적으로 관리하는 Agent를 구현했습니다.',
        ...
        '<strong>RAGAS 기반 RAG 품질 검증</strong>: RAGAS faithfulness 평가로 생성된 응답이 검색된 문서에 충실함을 정량적으로 검증했습니다.',
    ...
    results: [
        ...
        '<strong>RAG 품질 검증 달성</strong>: RAGAS faithfulness 평가로 생성 응답의 문서 충실도를 정량 검증하고, 전체 파이프라인의 안정성을 확보했습니다.',
        '<strong>End-to-End Observability</strong>: LangFuse v3 tracing으로 Agent 실행 경로, 토큰 소비, 도구 호출 패턴을 실시간 추적하여 디버깅 효율을 향상시켰습니다.'
    ],

    modal: {
        overview: 'LangGraph StateGraph + MCP Protocol 기반 한국 금융 데이터 분석 AI Agent입니다. DART/KRX 데이터를 자연어로 질의하고, Voyage AI 임베딩 + FAISS 벡터 검색으로 공시 문서 RAG를 수행합니다.',
        ...
        keyFeatures: [
            ...
            '<strong>RAGAS 품질 검증</strong>: 생성 응답의 문서 충실도 정량 검증',
        ...
        technicalImplementation: [
            ...
            '<strong>Testing</strong>: pytest, RAGAS',
...
export const llmObservabilityContent = {
    situation: `LLM Serving 환경에서 기존 웹 서버 메트릭(RPS, Latency)만으로는 LLM 특유의 성능 병목을 파악할 수 없었습니다. TTFT(Time To First Token), TPS(Tokens Per Second), Queue Depth 등 LLM 전용 메트릭이 부재하여 throughput만으로는 보이지 않는 queuing bottleneck을 발견할 수 없는 상황이었습니다.`,

    tasks: [
        '<strong>LLM 전용 메트릭 설계</strong>: TTFT, TPS, TPOT, Queue Depth 등 커스텀 Prometheus 메트릭을 설계하여 LLM Serving 성능을 다각도로 관측.',
        '<strong>부하 테스트 시나리오 설계</strong>: baseline부터 stress까지 단계별 시나리오로 LLM 서빙 성능을 정량 분석.',
        '<strong>Canary Metric 발굴</strong>: Throughput만으로는 드러나지 않는 성능 병목을 TTFT와 Queue Depth로 파악.'
    ],

    actions: [
        '<strong>LLM 전용 Prometheus 메트릭 설계</strong>: TTFT, TPS, TPOT, Queue Depth, Active Requests 등 LLM Serving 전용 메트릭을 Prometheus Histogram/Gauge로 구현했습니다.',
        '<strong>MonitoredSemaphore 구현</strong>: active_requests와 queue_depth를 실시간 추적하는 세마포어를 구현하여 동시 요청 수와 대기열 깊이를 측정했습니다.',
        '<strong>단계별 부하 테스트 시나리오 실행</strong>: Baseline, Concurrency Sweep, Sustained Load, Variable Prompt Length, Model Comparison 시나리오로 LLM 서빙 성능을 체계적으로 분석했습니다.',
        '<strong>Grafana 대시보드 구축</strong>: TTFT 분포, TPS 추이, Queue Depth, Active Requests 등을 실시간 시각화하는 대시보드를 구축했습니다.'
    ],

    results: [
        '<strong>TTFT Canary Metric 발굴</strong>: 동시 요청 증가 시 TTFT가 급격히 저하되는 queuing bottleneck을 발견하여, throughput만으로는 드러나지 않는 성능 병목을 canary metric으로 확인했습니다.',
        '<strong>LLM Serving Observability Stack 확립</strong>: LLM Serving 전용 메트릭 + Grafana 대시보드로 서빙 성능을 다각도로 관측할 수 있는 체계를 구축했습니다.',
        '<strong>정량적 부하 분석 체계</strong>: 단계별 부하 테스트로 LLM 서빙 환경의 성능 한계와 병목 지점을 정량적으로 파악할 수 있는 분석 프레임워크를 확립했습니다.'
    ],

    modal: {
        overview: 'LLM Serving 전용 Observability Stack입니다. TTFT, TPS, TPOT, Queue Depth 등 LLM 전용 Prometheus 메트릭을 설계하고, 단계별 부하 테스트 시나리오로 성능을 정량 분석했습니다. TTFT canary metric으로 throughput만으로는 드러나지 않는 queuing bottleneck을 탐지했습니다.',

        keyFeatures: [
            '<strong>LLM 전용 메트릭</strong>: TTFT, TPS, TPOT, Queue Depth, Active Requests 등 커스텀 Prometheus 메트릭',
            '<strong>MonitoredSemaphore</strong>: active_requests/queue_depth 실시간 추적',
            '<strong>단계별 부하 테스트</strong>: Baseline → Concurrency Sweep → Sustained Load → Variable Prompt Length → Model Comparison 시나리오',
            '<strong>Grafana 대시보드</strong>: LLM 서빙 성능 실시간 시각화',
            '<strong>TTFT Canary Metric</strong>: queuing bottleneck 조기 탐지'
        ],
        ...
        learningPoints: [
            ...
            'MonitoredSemaphore를 활용한 동시성 추적 패턴 습득',
            'Canary metric 개념으로 성능 병목을 조기에 탐지하는 경험'
        ]
    }
};
